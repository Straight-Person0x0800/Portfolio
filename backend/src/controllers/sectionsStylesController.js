const { SectionStyles } = require("../models"); // Import the NavStyles model
...
const getStyles = async (req, res) => {
  try {
    const styles = await SectionStyles.findAll();
...
const getStyle = async (req, res) => {
  try {
    const { id } = req.params;
    const style = await SectionStyles.findByPk(id);
...
const addStyle = async (req, res) => {
  try {
    ...
    const { error } = SectionStyles.validate(req.body.value);
