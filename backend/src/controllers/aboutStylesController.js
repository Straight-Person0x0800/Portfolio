const { AboutStyles } = require("../models"); // Import the NavStyles model
...
const getStyles = async (req, res) => {
  try {
    const styles = await AboutStyles.findAll();
...
const getStyle = async (req, res) => {
  try {
    const { id } = req.params;
    const style = await AboutStyles.findByPk(id);
...
const addStyle = async (req, res) => {
  try {
    ...
    const { error } = AboutStyles.validate(req.body.value);
