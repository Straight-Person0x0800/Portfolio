const { NavStyles } = require("../models"); // Import the NavStyles model
...
const getStyles = async (req, res) => {
  try {
    const styles = await NavStyles.findAll();
...
const getStyle = async (req, res) => {
  try {
    const { id } = req.params;
    const style = await NavStyles.findByPk(id);
...
const addStyle = async (req, res) => {
  try {
    ...
    const { error } = styleSchema.validate(req.body.value);
